...
# --- ADD THIS BLOCK ---
print(f"\n--- {__file__} Model Parameter Names ---")
...
# Optional: Check if head weights seem loaded (simple check)
try:
    # Check for example.py style naming (using add_module)
    head_param_name_example = 'head_0.fc.weight'
    if head_param_name_example in loaded_param_names:
         print(f"Parameter '{head_param_name_example}' found.")
         # print(model.state_dict()[head_param_name_example]) # Uncomment to see actual values
    else:
         print(f"WARNING: Parameter '{head_param_name_example}' NOT found.")
except Exception as e:
    print(f"Error checking head param: {e}")
# --- END ADD ---
...
# --- ADD THIS BLOCK ---
print("\n--- example.py Tokenizer Output ---")
print(f"Input IDs: {encoded_texts['input_ids'].tolist()}")
print(f"Attention Mask: {encoded_texts['attention_mask'].tolist()}")
print("----------------------------------\n")
# --- END ADD ---
